...
class convertSDDSToGDF():
    ...
    def sdds_to_gdf(self, filename, charge=None, mass=None):
        ...
        self.beam.read_SDDS_beam_file(self.global_parameters['master_subdir'] + '/' + beamfilename)
        gdftxtfilename = self.output_filename.replace('.sdds','.txt').strip('\"')
        self.beam.write_gdf_beam_file(self.global_parameters['master_subdir'] + '/' + gdftxtfilename, charge=charge, mass=mass)
        gdffilename = self.output_filename.replace('.sdds','.gdf').strip('\"')
        subprocess.call([r'C:\Program Files\General Particle Tracer\bin\asci2gdf', '-o', gdffilename, gdftxtfilename], cwd=self.global_parameters['master_subdir'])

if __name__ == "__main__":
    args = parser.parse_args()
    converter = convertSDDSToGDF()
    converter.sdds_to_gdf(args.filename, constants.elementary_charge, constants.proton_mass)
