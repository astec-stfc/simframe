...
class convertGDFToSDDS():
    ...
    def gdf_to_sdds(self, filename, middle=0, end=0):
        ...
        self.beam.read_gdf_beam_file(self.global_parameters['master_subdir'] + '/' + beamfilename)
        SDDSfilename = self.output_filename.replace('.gdf','.sdds').strip('\"')
        self.beam.write_SDDS_beam_file(self.global_parameters['master_subdir'] + '/' + SDDSfilename)
