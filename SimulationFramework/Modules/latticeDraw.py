...
class rectangleElement(pg.GraphicsObject):
    ...
    def generatePicture(self):
        ...
        p.fillRect(QtCore.QRectF(self.start,  -0.5*abs(self.width*self.scale), self.end - self.start, abs(self.width*self.scale)), color)
        p.end()
...
class lineElement(pg.GraphicsObject):
    ...
    def generatePicture(self):
        ...
        p.setBrush(pg.mkBrush(color))
        p.drawRect(QtCore.QRectF(self.start,  -0.5*abs(self.width*self.scale), self.end - self.start, abs(self.width*self.scale)))
        p.end()
    ...
    def defineElements(self):
        ...
        for name, elem in self.element_positions.items():
            ...
            if type in self.colorwidths:
                self.elements.append(rectangleElement(min([start,end]),max([start,end]), *self.colorwidths[type]))
                if end0 < start:
                    self.elements.append(lineElement(end0, start, *self.colorwidths['line']))
                if end > end0:
                    end0 = end
...
if __name__ == '__main__':
    ...
    import sys, os
    sys.path.append(os.path.abspath(__file__+'/../../../../ASTRA_COMPARISONRunner-HMCC/OnlineModel/'))
    from data import data
    dataObject = data.Data(settings_directory=os.path.abspath(__file__+'/../../../../ASTRA_COMPARISONRunner-HMCC/OnlineModel/'))
