...
class twiss(munch.Munch):
    ...
    @property
    def normal(self):
        return {p: getattr(self,p) for p in ('horizontal_emittance', 'alpha_x', 'beta_x', 'vertical_emittance', 'alpha_y', 'beta_y')}

    @property
    def corrected(self):
        return {p+'_corrected': getattr(self,p+'_corrected') for p in ('horizontal_emittance', 'alpha_x', 'beta_x', 'vertical_emittance', 'alpha_y', 'beta_y')}
    ...
    @property
    def beta_x(self):
        return self.beam.covariance(self.beam.x,self.beam.x) / self.horizontal_emittance
    @property
    def alpha_x(self):
        return -1*self.beam.covariance(self.beam.x,self.beam.xp) / self.horizontal_emittance
    @property
    def gamma_x(self):
        return self.beam.covariance(self.beam.xp,self.beam.xp) / self.horizontal_emittance
    @property
    def beta_y(self):
        return self.beam.covariance(self.beam.y,self.beam.y) / self.vertical_emittance
    @property
    def alpha_y(self):
        return -1*self.beam.covariance(self.beam.y,self.beam.yp) / self.vertical_emittance
    @property
    def gamma_y(self):
        return self.beam.covariance(self.beam.yp,self.beam.yp) / self.vertical_emittance
    @property
