...
class astraPlotWidget(QWidget):
    ...
    def changeScreen(self, i):
        ...
        self.beamFileName = (
            str(
                self.screenpositions[str(self.fileSelector.currentText())][
                    "screennames"
                ][self.screenSelector.currentIndex()]
            )
            + ".hdf5"
        )
    ...
    def updateScreenCombo(self):
        ...
        for i, s in enumerate(
            self.screenpositions[str(self.fileSelector.currentText())]["screennames"]
        ):
    ...
    def plotDataBeam(self):
        ...
        if self.chooseSliceWidth.isChecked():
            slices = np.arange(
                min(x), max(x) + self.sliceWidth.value(), self.sliceWidth.value()
            )
            self.sliceNumber.setValue(len(slices))
            self.sc.plothist(
                x, y, [slices, len(slices)], xLabel=xdict["axis"], yLabel=ydict["axis"]
            )
        else:
            self.sliceWidth.setValue(
                int(1e6 * ((max(x) - min(x)) / self.sliceNumber.value())) / 1e6
            )
            self.sc.plothist(
                x,
                y,
