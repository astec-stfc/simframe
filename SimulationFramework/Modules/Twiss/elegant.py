...
def read_elegant_floor_file(self, filename, offset=[0,0,0], rotation=[0,0,0], reset=True):
    ...
    self['x'] = twissData([np.round(x+offset[0], decimals = 6) for x in elegantData['X']], units='m')
    self['y'] = twissData([np.round(y+offset[1], decimals = 6) for y in elegantData['Y']], units='m')
    self['z'] = twissData([np.round(z+offset[2], decimals = 6) for z in elegantData['Z']], units='m')
    self['theta'] = twissData([np.round(theta+rotation[0], decimals = 6) for theta in elegantData['theta']], units='radians')
    self['phi'] = twissData([np.round(phi+rotation[1], decimals = 6) for phi in elegantData['phi']], units='radians')
    self['psi'] = twissData([np.round(psi+rotation[2], decimals = 6) for psi in elegantData['psi']], units='radians')
...
def read_elegant_twiss_files(self, filename, startS=0, reset=True):
    ...
    if isinstance(filename, (list, tuple)):
        ...
        self.append('sigma_cp', elegantData['Sdelta'] * cp / constants.elementary_charge)
