...
def read_elegant_twiss_files(self, filename, startS=0, reset=True):
    ...
    if isinstance(filename, (list, tuple)):
        ...
        self.alpha_x_beam.val = np.append(self.alpha_x_beam.val, elegantData["alphaxBeam"])
        self.alpha_y_beam.val = np.append(self.alpha_y_beam.val, elegantData["alphayBeam"])
        ...
        self.gamma_x.val = np.append(self.gamma_x.val, (1 + elegantData["alphax"] ** 2) / elegantData["betax"])
        self.gamma_y.val = np.append(self.gamma_y.val, (1 + elegantData["alphay"] ** 2) / elegantData["betay"])
        ...
        self.sigma_z.val = np.append(self.sigma_z.val, elegantData["St"] * (beta * constants.speed_of_light))
        # self.append('sigma_cp', elegantData['Sdelta'] * cp )
        self.sigma_cp.val = np.append(self.sigma_cp.val, elegantData["Sdelta"] * cp / constants.elementary_charge)
        ...
        self.element_name.val = np.append(self.element_name.val, elegantData["ElementName"])
        self.lattice_name.val = np.append(self.lattice_name.val, np.full(len(elegantData["ElementName"]), lattice_name))
        # ## BEAM parameters

        self.eta_x_beam.val = np.append(self.eta_x_beam.val, elegantData["s16"] / (elegantData["s6"] ** 2))
        self.eta_xp_beam.val = np.append(self.eta_xp_beam.val, elegantData["s26"] / (elegantData["s6"] ** 2))
        self.eta_y_beam.val = np.append(self.eta_y_beam.val, elegantData["s36"] / (elegantData["s6"] ** 2))
        self.eta_yp_beam.val = np.append(self.eta_yp_beam.val, elegantData["s46"] / (elegantData["s6"] ** 2))
