...
def is_gdf_file(filename):
    with open(
        filename, "rb"
    ) as f:  # Important to open in binary mode 'b' to work cross platform
        ...
        with open(
            ...
            while True:
                ...
                if lastarr and not arr:
                    ...
                    grab_group_number += 1
                    grab_group = self.create_grab_group(
                        "datagrab_" + str(grab_group_number)
                    )
                    grab_group.attrs["grab_number"] = grab_group_number
                    data_group = grab_group.create_group("data")
                    param_group = grab_group.create_group("param")
                    ...
                    if dattype == t_dbl:
                        if (size % 8) != 0:
                            raise RuntimeWarning(
                                "Tried to save an array of doubles, but the array size is not consistant with that of doubles."
                            )
                        value = np.fromfile(
                            f, dtype=np.dtype("f8"), count=int(size / 8)
                        )
