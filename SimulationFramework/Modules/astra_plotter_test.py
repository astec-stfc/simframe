...
class astraPlotWidget(QWidget):
    ...
    def __init__(self, directory=".", **kwargs):
        ...
        for param in self.sliceParams:
            ...
            self.slicePlotWidgetGraphicsLayout.ci.addItem(
                axis, row=0, col=col, rowspan=1, colspan=1
            )
        ...
        if directory == None or directory == False:
            self.directory = str(
                QFileDialog.getExistingDirectory(
                    self, "Select Directory", self.directory, QFileDialog.ShowDirsOnly
                )
            )
        else:
            self.directory = directory
    ...
    def getScreenFiles(self):
        ...
        files = glob.glob(
            self.directory + "/*[.][0-9][0-9][0-9][0-9][.][0-9][0-9][0-9]"
        )
        filenames = [
        ...
        for f, r in list(set(zip(filenames, runnumber))):
            files = glob.glob(
                self.directory + "/" + f + ".[0-9][0-9][0-9][0-9].[0-9][0-9][0-9]"
            )
            screenpositions = [
                re.search(f + "\.(\d\d\d\d)\.\d\d\d", s).group(1) for s in files
            ]
    ...
    def updateFileCombo(self):
        ...
        if len(self.screenpositions) > 0:
            ...
            sortedscreennames = screenfirstpos[
                np.argsort(np.array(screenfirstpos)[:, 1])
            ]
    ...
    def changeScreen(self, i):
        ...
        self.beamFileName = (
            str(self.fileSelector.currentText())
            + "."
            + str(self.screenSelector.currentText())
            + "."
            + str(run)
        )
    ...
    def updateScreenCombo(self):
        ...
        i = -1
        for s in self.screenpositions[str(self.fileSelector.currentText())][
            "screenpositions"
        ]:
    ...
    def loadDataFile(self):
        if self.plotType == "Twiss":
            ...
            if hasattr(self, "beamFileName") and os.path.isfile(
                self.directory + "/" + self.beamFileName
            ):
                # starttime = time.time()
                self.beam.read_astra_beam_file(
                    self.directory + "/" + self.beamFileName, normaliseZ=True
                )
    ...
    def plotDataTwiss(self):
        for entry in self.twissplotLayout:
            if entry == "next_row":
                ...
                self.twissPlots[entry["name"]].setData(
                    x=x, y=y, pen=mkPen("b", width=3)
                )
    ...
    def plotDataBeam(self):
        ...
        if hasattr(self, "sc"):
            self.sc.plothist(
                x, y, self.histogramBins, xLabel=xdict["axis"], yLabel=ydict["axis"]
            )
        ...
        for param in self.sliceParams:
            if self.slicePlotCheckbox[param["name"]].isChecked():
                ...
                with open(
                    self.directory
                    + "/"
                    + self.fileSelector.currentText()
                    + "."
                    + self.screenSelector.currentText()
                    + "."
                    + param["name"]
                    + "_slices.csv",
                    "w",
