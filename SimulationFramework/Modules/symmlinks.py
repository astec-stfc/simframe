...
def has_symlink_privilege():
    privs = {p.get_name(): p.is_enabled() for p in get_privilege_information().get_array()}
    if 'SeCreateSymbolicLinkPrivilege' in privs:
        return privs['SeCreateSymbolicLinkPrivilege']
    else:
        return False

def main():
    print(has_symlink_privilege())
