...
class twiss(munch.Munch):
    ...
    def __getitem__(self, key):
        if (
            key in super(twiss, self).__getitem__("data")
            and super(twiss, self).__getitem__("data") is not None
        ):
    ...
    def append(self, array, data):
        self._data[array] = twissData(
            np.concatenate([self._data[array], data]), units=self._data[array].units
        )
    ...
    def read_elegant_floor_file(
        ...
        self["x"] = twissData(
            [np.round(x + offset[0], decimals=6) for x in self.elegant["X"]], units="m"
        )
        self["y"] = twissData(
            [np.round(y + offset[1], decimals=6) for y in self.elegant["Y"]], units="m"
        )
        self["z"] = twissData(
            [np.round(z + offset[2], decimals=6) for z in self.elegant["Z"]], units="m"
        )
        self["theta"] = twissData(
            [
                np.round(theta + rotation[0], decimals=6)
                for theta in self.elegant["theta"]
            ],
            units="radians",
        )
        self["phi"] = twissData(
            [np.round(phi + rotation[1], decimals=6) for phi in self.elegant["phi"]],
            units="radians",
        )
        self["psi"] = twissData(
            [np.round(psi + rotation[2], decimals=6) for psi in self.elegant["psi"]],
            units="radians",
        )
        xyz = list(zip(self["x"], self["y"], self["z"]))
        thetaphipsi = list(zip(self["phi"], self["psi"], self["theta"]))
        return list(
            zip(self.elegant["ElementName"], xyz[-1:] + xyz[:-1], xyz, thetaphipsi)
        )[1:]
    ...
    def read_elegant_twiss_files(self, filename, startS=0, reset=True):
        ...
        if isinstance(filename, (list, tuple)):
            ...
            self.append(
                "gamma_x", (1 + self.elegant["alphax"] ** 2) / self.elegant["betax"]
            )
            self.append("beta_y", self.elegant["betay"])
            self.append("alpha_y", self.elegant["alphay"])
            self.append(
                "gamma_y", (1 + self.elegant["alphay"] ** 2) / self.elegant["betay"]
            )
            ...
            self.append(
                "sigma_z", self.elegant["St"] * (beta * constants.speed_of_light)
            )
            self.append("sigma_cp", self.elegant["Sdelta"] * cp)
            self.append(
                "sigma_cp_eV", self.elegant["Sdelta"] * cp / constants.elementary_charge
            )
    ...
    def read_astra_twiss_files(self, filename, reset=True):
        ...
        if isinstance(filename, (list, tuple)):
            ...
            xemit = (
                np.loadtxt(filename, unpack=False)
    ...
    def interpret_astra_data(self, xemit, yemit, zemit):
        ...
        # print('astra = ', (rms_e)[-1)
        self.append(
            "mux", integrate.cumtrapz(x=self["z"], y=1 / self["beta_x"], initial=0)
        )
        self.append(
            "muy", integrate.cumtrapz(x=self["z"], y=1 / self["beta_y"], initial=0)
        )
    ...
    def read_gdf_twiss_files(self, filename=None, gdfbeam=None, reset=True):
        ...
        if os.path.isfile(filename):
            ...
            self.append(
                "sigma_cp_eV",
                (gdfbeamdata.stdG / gdfbeamdata.avgG)
                * cp
                / constants.elementary_charge,
            )
            self.append(
                "sigma_p",
                (gdfbeamdata.stdG / gdfbeamdata.avgG) * cp / constants.speed_of_light,
            )
    ...
    def interpolate(self, z=None, value="z", index="z"):
        f = interpolate.interp1d(
            self[index], self[value], kind="linear", fill_value="extrapolate"
        )
        if z is None:
            return f
        else:
