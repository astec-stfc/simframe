...
class beamGroup(munch.Munch):
    ...
    def __getitem__(self, key):
        ...
        if (
            key in super(beamGroup, self).__getitem__("beams")
            and super(beamGroup, self).__getitem__("beams") is not None
        ):
...
class beam(munch.Munch):
    ...
    def __getitem__(self, key):
        if (
            key in super(beam, self).__getitem__("_beam")
            and super(beam, self).__getitem__("_beam") is not None
        ):
    ...
    def write_SDDS_file(self, filename, ascii=False, xyzoffset=[0, 0, 0]):
        ...
        zoffset = xyzoffset[
            2
        ]  # Don't think I need this because we are using t anyway...
        self.sddsindex += 1
    ...
    def convert_csrtrackfile_to_astrafile(self, csrtrackfile, astrafile):
        ...
        array = np.array(
            [x, y, z, cpx, cpy, cpz, clock, charge, index, status]
        ).transpose()
    ...
    def write_astra_beam_file(
        self, file, index=1, status=5, charge=None, normaliseZ=False
    ):
        ...
        statusvector = (
            ...
            if "status" in self._beam
                ...
                else np.full(len(self.x), status)
            )
        )
        ...
        if self["longitudinal_reference"] == "t":
            zvector = [
                ...
                for status, z, t in zip(statusvector, self.z, self.t)
            ]
        else:
            zvector = self.z
        ...
        clockvector = [
            1.0e9 * t for status, z, t, Bz in zip(statusvector, self.z, self.t, self.Bz)
        ]
    ...
    def write_vsim_beam_file(self, file, normaliseT=False):
        if len(self._beam["charge"]) == len(self.x):
            ...
            chargevector = np.full(
                len(self.x), self._beam["total_charge"] / len(self.x)
            )
        if normaliseT:
        ...
        """ take the rms - if the rms is 0 set it to 1, so we don't get a divide by error """
        np.savetxt(
            file, array, fmt=("%.12e", "%.12e", "%.12e", "%.12e", "%.12e", "%.12e")
        )

    def write_gdf_beam_file(
        self, filename, normaliseX=False, normaliseZ=False, cathode=False
    ):
    ...
    def read_HDF5_beam_file(self, filename, local=False):
        ...
        with h5py.File(filename, "r") as h5file:
            ...
            x, y, z, cpx, cpy, cpz, t, charge = np.array(
                h5file.get("beam/beam")
            ).transpose()
            cp = np.sqrt(cpx**2 + cpy**2 + cpz**2)
            self._beam["x"] = x
            self._beam["y"] = y
            ...
            if local == True:
                self.rotate_beamXZ(
                    self["rotation"], preOffset=self["starting_position"]
                )
