...
parser = argparse.ArgumentParser(
    description="Analyse genesis output file and return relevant parameters"
)
...
def main():
    ...
    for d in dirs:
        ...
        fig2, ax = plt.subplots(3, 2, figsize=(10, 10))
        ...
        stdpeakIPDF = (max(peakIPDF[indexes2]) - min(peakIPDF[indexes2])) / np.mean(
            peakIPDF[indexes2]
        )  # Flat-top in the distribution!
        ...
        if ymax - ymin < 5:
            ...
            ymin = np.mean(p) - 2.5
        ax[1][0].hist2d(
            t, p, bins=(50, 50), cmap=plt.cm.jet, range=[[min(t), max(t)], [ymin, ymax]]
        )
