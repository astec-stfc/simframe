...
class Optimise_transverse(runEle.fitnessFunc):
    ...
    def __init__(self, lattice="Lattices/clara400_v12_v3.def", scaling=5):
        ...
        self.parameter_names = [
            q for q in self.framework.getElementType("quadrupole", "objectname")
        ]
        self.parameters = [
            [q, "k1l"]
            for q in self.framework.getElementType("quadrupole", "objectname")
        ]
        ...
        if self.POST_INJECTOR and CREATE_BASE_FILES:
            self.optfunc = partial(
                self.OptimisingFunction,
                scaling=scaling,
                post_injector=self.POST_INJECTOR,
                basefiles="../CLARA/basefiles_" + str(scaling) + "/",
                CLARA_dir=CLARA_dir,
                lattice=self.lattice_file,
            )
        else:
            self.optfunc = partial(
                self.OptimisingFunction,
    ...
    def calculateBeamParameters(self):
        ...
        constraintsListQuads = {
            "max_k": {
                ...
                "value": [
                    abs(k / l)
                    for k, l, n in zip(quadkls, quadlengths, quadnames)
                    if not "FMS" in n
                ],
                "limit": 2.5,
                "weight": 10,
        ...
        tdc_screen_position = self.framework["CLA-S07-DIA-SCR-03-W"]["position_start"][
            2
        ]
        ...
        constraintsListS07 = {
            ...
            "dechirper_sigma_x": {
                "type": "lessthan",
                "value": 1e3 * twiss.interpolate(dechirper_position, "sigma_x"),
                "limit": 0.1,
                "weight": 10,
            },
            "dechirper_sigma_y": {
                "type": "lessthan",
                "value": 1e3 * twiss.interpolate(dechirper_position, "sigma_y"),
                "limit": 0.1,
                "weight": 10,
            },
            "dechirper_sigma_xy": {
    ...
    def OptimisingFunction(self, inputargs, *args, **kwargs):
        ...
        fit = self.setup_lattice(
            self.inputlist, dir, *args, changes=self.changes, **kwargs
        )
    ...
    def Nelder_Mead(self, best=None, step=0.1):
        ...
        res = nelder_mead(
            self.optfunc, best, step=step, max_iter=300, no_improv_break=100
        )
        print(res)
