...
class mad8Lattice(frameworkLattice):
    ...
    def write(self):
        self.lattice_file = (
            self.global_parameters["master_subdir"] + "/" + self.objectname + ".mad"
        )
        saveFile(self.lattice_file, self.writeElements())
        self.command_file = (
            self.global_parameters["master_subdir"] + "/" + self.objectname + ".mff"
        )
    ...
    def preProcess(self):
        ...
        self.commandFile = mad8TrackFile(
            ...
            mad8beamfilename=self.objectname + ".tfs",
    ...
    def hdf5_to_tfs(self, prefix=""):
        ...
        self.global_parameters["beam"].write_TFS_file(
            self.global_parameters["master_subdir"] + "/" + tfsbeamfilename,
...
class mad8CommandFile(frameworkObject):
    ...
    def addCommand(self, name=None, **kwargs):
        ...
        command = frameworkCommand(
            name, global_parameters=self.global_parameters, **kwargs
        )
...
class mad8TrackFile(mad8CommandFile):
    def __init__(
        ...
        **kwargs
        ...
        self.betax = (
            betax if betax is not None else self.global_parameters["beam"].beta_x
        )
        self.betay = (
            betay if betay is not None else self.global_parameters["beam"].beta_y
        )
        self.alphax = (
            alphax if alphax is not None else self.global_parameters["beam"].alpha_x
        )
        self.alphay = (
            alphay if alphay is not None else self.global_parameters["beam"].alpha_y
        )
        self.addCommand(
            type="global_settings"
        )  # , mpi_io_read_buffer_size=262144, mpi_io_write_buffer_size=262144)
        self.addCommand(
            type="run_setup",
