...
gpt_generator_keywords = {
    "keywords": ["image_filename", "image_calibration_x", "image_calibration_y"],
}
generator_keywords = {
    "defaults": {
        "clara_400_3ps": {
        ...
        "clara_400_2ps_Gaussian": {
            ...
            "distribution_type_pz": "i",
            "thermal_emittance": 0.9e-3,
            "distribution_type_x": "2DGaussian",
            "sigma_x": 0.25e-3,
            "distribution_type_y": "2DGaussian",
            "sigma_y": 0.25e-3,
            "guassian_cutoff_x": 3,
            "guassian_cutoff_y": 3,
            "guassian_cutoff_z": 3,
...
class frameworkGenerator(Munch):
    ...
    def charge(self):
        return (
            float(self["charge"])
            if "charge" in self and self["charge"] is not None
            else 250e-12
        )
    ...
    def objectname(self):
        return (
            self["name"]
            if "name" in self.keys() and self["name"] is not None
            else "generator"
        )
    ...
    def write(self):
        ...
        if str(self["thermal_emittance"]) == "None":
            ...
            framework_dict.update(
                {
                    "le": {
                        "value": 1e-3 * self.thermal_kinetic_energy,
                        "default": 0.62e-3,
                    }
                }
            ),
        ...
        for k in self.allowedKeyWords:
            ...
            if klower not in [fk.lower() for fk in framework_dict.keys()] and k not in [
                ...
                if (
                    getattr(self, klower) is not None
                    and getattr(self, klower) != "None"
                ):
                    try:
...
E0 = """
            + str(self.thermal_kinetic_energy)
            + """;
G = 1-qe*E0/(me * c * c);
GB = sqrt(G^2 - 1);
Qtot = """
            + str(-1e12 * self.charge)
            + """e-12;
npart = """
            + str(self.particles)
            + """;
...
"""
    ...
    def _gaussian_distribution(
        self, distname: str, variable: str, left_cutoff=3, right_cutoff=3, **kwargs
    ):
        return (
            distname
            + '( "beam", "g", 0, '
    ...
    def generate_radial_distribution(self):
        ...
        if self.distribution_type_x == "image" or self.distribution_type_y == "image":
            ...
            output = (
                ...
                + str(image_filename)
                + '", '
                + str(image_calibration_x)
                + ", "
                + str(image_calibration_y)
                + ") ;\n"
            )
            return output
        elif (self.sigma_x != self.sigma_y) and (
            self.distribution_type_x == self.distribution_type_y
        ):
            ...
            output += (
                self._distribution(
                    ...
                    "radius",
                    left_cutoff=0,
                    right_cutoff=self.guassian_cutoff_x,
                )
                + "\n"
            )
...
"""
    ...
    def generate_thermal_emittance(self):
        if self.distribution_type_x == "image" or self.distribution_type_x == "image":
            ...
            output = (
                """setGBxemittance("beam", ("""
                + str(thermal_emittance)
                + """*radius_x)) ;
"""
            )
            output += (
                """setGByemittance("beam", ("""
                + str(thermal_emittance)
                + """*radius_y)) ;
"""
            )
...
"""
    ...
    def generate_longitudinal_distribution(self):
        if self.distribution_type_z.lower() in ["g", "gaussian"]:
            ...
            output = (
                """tlen = """ + str(1e12 * self.plateau_bunch_length) + """e-12;\n"""
            )
        output += (
            self._distribution(
                "distribution_type_z",
                "settdist",
                "tlen",
                left_cutoff=self.guassian_cutoff_z,
                right_cutoff=self.guassian_cutoff_z,
                left_multiplier=0,
                right_multiplier=1,
            )
            + "\n"
...
"""
    ...
    def postProcess(self):
        ...
        HDF5filename = "laser.hdf5"
        self.global_parameters["beam"]["status"] = np.full(
            len(self.global_parameters["beam"]["x"]), -1
        )
        self.global_parameters["beam"].write_HDF5_beam_file(
            self.global_parameters["master_subdir"] + "/" + HDF5filename,
            centered=False,
